...
{
    if (members.Any(member => member.name == "Client" && member.map == "customs" &&
        Math.Abs(member.x - 44.5) < 0.01 && Math.Abs(member.qy - Math.Sqrt(0.5)) < 0.0001 && Math.Abs(member.qw - Math.Sqrt(0.5)) < 0.0001))
...
{
    if (members.Any(member => member.name == "Host" && member.map == "customs" &&
        Math.Abs(member.z - 99.25) < 0.01 && Math.Abs(member.qy + Math.Sqrt(0.5)) < 0.0001 && Math.Abs(member.qw - Math.Sqrt(0.5)) < 0.0001))
...
host.UpdatePose("customs", 10, 20, 99.25, 0, -Math.Sqrt(0.5), 0, Math.Sqrt(0.5));
client.UpdatePose("customs", 44.5, 2, 3, 0, Math.Sqrt(0.5), 0, Math.Sqrt(0.5));
...
Console.WriteLine($"Direct squad smoke test passed on UDP {port}: encrypted host/client positions and headings exchanged; wrong password rejected.");
